/* ACCESS_BREACH — password-cracking terminal puzzle. Reuses the
   existing WORD_LIST dataset (data-words.js), same as Hangman/Anagram
   Solver, rather than a separate word bank - it's already large enough
   at every relevant length (thousands of words from 4-9 letters).

   Feedback is a single character-position-match COUNT (how many
   letters of a guessed word are in the same position as the real
   password), not per-letter color-coding - that's Wordle's mechanic,
   this is a different, older one. Every click is a full-word guess
...
const BREACH_LENGTH_RANGES = { short: [4, 5], medium: [6, 7], long: [8, 9] };
const BREACH_JUNK_CHARS = "!@#$%^&*_+-=~<>?/\\|;:,.".split("");
const BREACH_BRACKET_STYLES = [["[", "]"], ["(", ")"], ["<", ">"], ["{", "}"]];
const BREACH_STATS_KEY = "gama_access_breach_stats";

let breachGrid = [];
let breachConsumedBrackets = new Set();
let breachWordPlacements = {};
let breachBracketPlacements = [];
let breachTarget = "";
let breachCandidates = [];   // words still guessable (not yet rejected/removed)
let breachRejected = new Set();
...
function pickBreachWords(wordList, length, count, rng = Math.random) {
  const candidates = wordList.filter((w) => w.length === length && /^[a-z]+$/.test(w));
...
function generateBreachGrid(colWidth, lineCount, numCols, words, bracketCount, rng = Math.random) {
  ...
  return { grid, wordPlacements, bracketPlacements };
}

/* --- Persistent stats --- */

function loadBreachStats() {
  try {
    const raw = localStorage.getItem(BREACH_STATS_KEY);
...
function saveBreachStats(stats) {
  try { localStorage.setItem(BREACH_STATS_KEY, JSON.stringify(stats)); } catch { /* unavailable - just won't persist */ }
}

function renderBreachStats() {
  const stats = loadBreachStats();
  document.getElementById("breach-stats").textContent = `puzzles solved: ${stats.solved} // console lockouts: ${stats.lockouts}`;
}

/* --- Setup + rendering --- */

function newBreachGame() {
  const difficulty = document.getElementById("breach-difficulty").value;
  const [min, max] = BREACH_LENGTH_RANGES[difficulty];
  ...
  breachBracketPlacements = result.bracketPlacements;
  breachTarget = target;
  breachCandidates = Object.keys(breachWordPlacements);
  breachRejected = new Set();
  breachConsumedBrackets = new Set();
  ...
  for (let l = 0; l < BREACH_LINE_COUNT; l++) {
    ...
    for (let p = 0; p < BREACH_COL_WIDTH; p++) {
      ...
      if (cellData.type === "word") {
        ...
        // Give the target a real visual reveal once the round ends -
        // without this, on a LOSS the target was never added to
        // breachRejected (it was never guessed), so it would render
        // identically to every other untried decoy and the grid itself
        // would give no closure beyond the status-line text.
        if (breachOver && isTarget) {
          span.style.color = "var(--phosphor)";
          span.style.background = "rgba(57,255,20,0.25)";
          span.style.textDecoration = "none";
        }
      } else if (cellData.type === "bracket" || cellData.type === "bracket-fill") {
        const consumed = breachConsumedBrackets.has(cellData.id);
        span.style.color = consumed ? "var(--phosphor-dim-text)" : "var(--amber)";
        if (!consumed && !breachOver && !breachBusy) {
          span.style.cursor = "pointer";
          span.addEventListener("click", () => executeBreachBracket(cellData.id));
        }
      } else {
        span.style.color = "var(--phosphor-dim-text)";
      }
  ...
  setTimeout(() => {
    ...
    if (word === breachTarget) {
      ...
      if (breachAttemptsUsed >= BREACH_MAX_ATTEMPTS) {
        ...
        result.textContent = `> ${matches}/${word.length} correct - ${breachAttemptsRemaining()} attempt${breachAttemptsRemaining() === 1 ? "" : "s"} left`;
        GAMA.say("idle");
      }
    }
    renderBreachTerminal();
...
/* --- Bracket bonus: 50/50 remove a wrong candidate from play, or
   restore a used attempt. Single-use per bracket. Skipped if there's
   nothing to remove or nothing to restore (falls through to the other
...
function executeBreachBracket(id) {
  ...
  let wantRemove = Math.random() < 0.5;
  if (wantRemove && removableCandidates.length === 0) wantRemove = false;
  if (!wantRemove && !canRestore) wantRemove = removableCandidates.length > 0;
...
/* --- Escape restarts, but counts as a loss (matches the source
   mechanic's anti-restart-spam intent) rather than a neutral reset. --- */

function breachIsTypingElsewhere() {
...
document.addEventListener("keydown", (e) => {
  if (e.key !== "Escape" || !document.getElementById("breach-terminal") || breachIsTypingElsewhere()) return;
  if (breachOver || breachBusy) return;
  breachOver = true;
  const stats = loadBreachStats();
  stats.lockouts++;
  saveBreachStats(stats);
  document.getElementById("breach-result").textContent = `> connection aborted (counts as a lockout). password was: ${breachTarget.toUpperCase()}`;
  renderBreachStats();
  renderBreachTerminal();
  GAMA.say("error");
  e.preventDefault();
...
document.addEventListener("DOMContentLoaded", () => {
  if (document.getElementById("breach-terminal")) newBreachGame();
});
