/* SYMBOL_INDEX data - curated common Unicode symbols with names, HTML entities,
   and codepoints. Names are the official Unicode character names (title-cased)
   so search-by-description works (e.g. "arrow", "heart", "check"). */
const SYMBOL_DATA = [
  ["Arrows", [
    ["←", "Leftwards Arrow", "&larr;", 8592],
    ["↑", "Upwards Arrow", "&uarr;", 8593],
    ["→", "Rightwards Arrow", "&rarr;", 8594],
    ["↓", "Downwards Arrow", "&darr;", 8595],
    ["↔", "Left Right Arrow", "&harr;", 8596],
    ["↕", "Up Down Arrow", "&#8597;", 8597],
    ["↖", "North West Arrow", "&#8598;", 8598],
    ["↗", "North East Arrow", "&#8599;", 8599],
    ["↘", "South East Arrow", "&#8600;", 8600],
    ["↙", "South West Arrow", "&#8601;", 8601],
    ["↚", "Leftwards Arrow With Stroke", "&#8602;", 8602],
    ["↛", "Rightwards Arrow With Stroke", "&#8603;", 8603],
    ["↜", "Leftwards Wave Arrow", "&#8604;", 8604],
    ["↝", "Rightwards Wave Arrow", "&#8605;", 8605],
    ["↞", "Leftwards Two Headed Arrow", "&#8606;", 8606],
    ["↟", "Upwards Two Headed Arrow", "&#8607;", 8607],
    ["↠", "Rightwards Two Headed Arrow", "&#8608;", 8608],
    ["↡", "Downwards Two Headed Arrow", "&#8609;", 8609],
    ["↢", "Leftwards Arrow With Tail", "&#8610;", 8610],
    ["↣", "Rightwards Arrow With Tail", "&#8611;", 8611],
    ["↤", "Leftwards Arrow From Bar", "&#8612;", 8612],
    ["↥", "Upwards Arrow From Bar", "&#8613;", 8613],
    ["↦", "Rightwards Arrow From Bar", "&#8614;", 8614],
    ["↧", "Downwards Arrow From Bar", "&#8615;", 8615],
    ["↨", "Up Down Arrow With Base", "&#8616;", 8616],
    ["↩", "Leftwards Arrow With Hook", "&#8617;", 8617],
    ["↰", "Upwards Arrow With Tip Leftwards", "&#8624;", 8624],
    ["↱", "Upwards Arrow With Tip Rightwards", "&#8625;", 8625],
    ["↶", "Anticlockwise Top Semicircle Arrow", "&#8630;", 8630],
    ["↷", "Clockwise Top Semicircle Arrow", "&#8631;", 8631],
    ["↖", "North West Arrow", "&#8598;", 8598],
    ["↗", "North East Arrow", "&#8599;", 8599],
    ["↘", "South East Arrow", "&#8600;", 8600],
    ["↙", "South West Arrow", "&#8601;", 8601],
  ]],
  ["Math & Logic", [
    ["±", "Plus-Minus Sign", "&plusmn;", 177],
    ["×", "Multiplication Sign", "&times;", 215],
    ["÷", "Division Sign", "&divide;", 247],
    ["−", "Minus Sign", "&#8722;", 8722],
    ["≈", "Almost Equal To", "&asymp;", 8776],
    ["≠", "Not Equal To", "&ne;", 8800],
    ["≤", "Less-Than Or Equal To", "&le;", 8804],
    ["≥", "Greater-Than Or Equal To", "&ge;", 8805],
    ["≪", "Much Less-Than", "&#8810;", 8810],
    ["≫", "Much Greater-Than", "&#8811;", 8811],
    ["∞", "Infinity", "&infin;", 8734],
    ["√", "Square Root", "&radic;", 8730],
    ["∛", "Cube Root", "&#8731;", 8731],
    ["∜", "Fourth Root", "&#8732;", 8732],
    ["∑", "N-Ary Summation", "&sum;", 8721],
    ["∏", "N-Ary Product", "&prod;", 8719],
    ["∫", "Integral", "&#8747;", 8747],
    ["∬", "Double Integral", "&#8748;", 8748],
    ["∂", "Partial Differential", "&part;", 8706],
    ["∇", "Nabla", "&nabla;", 8711],
    ["∆", "Increment", "&#8710;", 8710],
    ["∎", "End Of Proof", "&#8718;", 8718],
    ["∀", "For All", "&forall;", 8704],
    ["∃", "There Exists", "&exist;", 8707],
    ["∄", "There Does Not Exist", "&#8708;", 8708],
    ["∅", "Empty Set", "&empty;", 8709],
    ["∈", "Element Of", "&isin;", 8712],
    ["∉", "Not An Element Of", "&notin;", 8713],
    ["⊂", "Subset Of", "&sub;", 8834],
    ["⊃", "Superset Of", "&sup;", 8835],
    ["⊆", "Subset Of Or Equal To", "&sube;", 8838],
    ["⊇", "Superset Of Or Equal To", "&supe;", 8839],
    ["∪", "Union", "&cup;", 8746],
    ["∩", "Intersection", "&cap;", 8745],
    ["∧", "Logical And", "&and;", 8743],
    ["∨", "Logical Or", "&or;", 8744],
    ["¬", "Not Sign", "&not;", 172],
    ["∴", "Therefore", "&#8756;", 8756],
    ["∵", "Because", "&#8757;", 8757],
    ["∝", "Proportional To", "&#8733;", 8733],
    ["≅", "Approximately Equal To", "&#8773;", 8773],
    ["≡", "Identical To", "&equiv;", 8801],
    ["°", "Degree Sign", "&deg;", 176],
    ["′", "Prime", "&prime;", 8242],
    ["″", "Double Prime", "&Prime;", 8243],
  ]],
  ["Greek Letters", [
    ["Α", "Greek Capital Letter Alpha", "&Alpha;", 913],
    ["Β", "Greek Capital Letter Beta", "&Beta;", 914],
    ["Γ", "Greek Capital Letter Gamma", "&Gamma;", 915],
    ["Δ", "Greek Capital Letter Delta", "&Delta;", 916],
    ["Ε", "Greek Capital Letter Epsilon", "&Epsilon;", 917],
    ["Ζ", "Greek Capital Letter Zeta", "&Zeta;", 918],
    ["Η", "Greek Capital Letter Eta", "&Eta;", 919],
    ["Θ", "Greek Capital Letter Theta", "&Theta;", 920],
    ["Ι", "Greek Capital Letter Iota", "&Iota;", 921],
    ["Κ", "Greek Capital Letter Kappa", "&Kappa;", 922],
    ["Λ", "Greek Capital Letter Lamda", "&Lambda;", 923],
    ["Μ", "Greek Capital Letter Mu", "&Mu;", 924],
    ["Ν", "Greek Capital Letter Nu", "&Nu;", 925],
    ["Ξ", "Greek Capital Letter Xi", "&Xi;", 926],
    ["Ο", "Greek Capital Letter Omicron", "&Omicron;", 927],
    ["Π", "Greek Capital Letter Pi", "&Pi;", 928],
    ["Ρ", "Greek Capital Letter Rho", "&Rho;", 929],
    ["α", "Greek Small Letter Alpha", "&alpha;", 945],
    ["β", "Greek Small Letter Beta", "&beta;", 946],
    ["γ", "Greek Small Letter Gamma", "&gamma;", 947],
    ["δ", "Greek Small Letter Delta", "&delta;", 948],
    ["ε", "Greek Small Letter Epsilon", "&epsilon;", 949],
    ["ζ", "Greek Small Letter Zeta", "&zeta;", 950],
    ["η", "Greek Small Letter Eta", "&eta;", 951],
    ["θ", "Greek Small Letter Theta", "&theta;", 952],
    ["ι", "Greek Small Letter Iota", "&iota;", 953],
    ["κ", "Greek Small Letter Kappa", "&kappa;", 954],
    ["λ", "Greek Small Letter Lamda", "&lambda;", 955],
    ["μ", "Greek Small Letter Mu", "&mu;", 956],
    ["ν", "Greek Small Letter Nu", "&nu;", 957],
    ["ξ", "Greek Small Letter Xi", "&xi;", 958],
    ["ο", "Greek Small Letter Omicron", "&omicron;", 959],
    ["π", "Greek Small Letter Pi", "&pi;", 960],
    ["ρ", "Greek Small Letter Rho", "&rho;", 961],
    ["ς", "Greek Small Letter Final Sigma", "&#962;", 962],
    ["σ", "Greek Small Letter Sigma", "&sigma;", 963],
    ["τ", "Greek Small Letter Tau", "&tau;", 964],
    ["υ", "Greek Small Letter Upsilon", "&upsilon;", 965],
    ["φ", "Greek Small Letter Phi", "&phi;", 966],
    ["χ", "Greek Small Letter Chi", "&chi;", 967],
    ["ψ", "Greek Small Letter Psi", "&psi;", 968],
    ["ω", "Greek Small Letter Omega", "&omega;", 969],
  ]],
  ["Currency", [
    ["$", "Dollar Sign", "&dollar;", 36],
    ["€", "Euro Sign", "&euro;", 8364],
    ["£", "Pound Sign", "&pound;", 163],
    ["¥", "Yen Sign", "&yen;", 165],
    ["¢", "Cent Sign", "&cent;", 162],
    ["₿", "Bitcoin Sign", "&#8383;", 8383],
    ["₹", "Indian Rupee Sign", "&#8377;", 8377],
    ["₩", "Won Sign", "&#8361;", 8361],
    ["₫", "Dong Sign", "&#8363;", 8363],
    ["₺", "Turkish Lira Sign", "&#8378;", 8378],
    ["₦", "Naira Sign", "&#8358;", 8358],
    ["₴", "Hryvnia Sign", "&#8372;", 8372],
    ["₪", "New Sheqel Sign", "&#8362;", 8362],
    ["฿", "Thai Currency Symbol Baht", "&#3647;", 3647],
    ["₭", "Kip Sign", "&#8365;", 8365],
    ["₱", "Peso Sign", "&#8369;", 8369],
  ]],
  ["Punctuation & Typography", [
    ["—", "Em Dash", "&mdash;", 8212],
    ["–", "En Dash", "&ndash;", 8211],
    ["…", "Horizontal Ellipsis", "&hellip;", 8230],
    ["“", "Left Double Quotation Mark", "&ldquo;", 8220],
    ["”", "Right Double Quotation Mark", "&rdquo;", 8221],
    ["‘", "Left Single Quotation Mark", "&lsquo;", 8216],
    ["’", "Right Single Quotation Mark", "&rsquo;", 8217],
    ["•", "Bullet", "&bull;", 8226],
    ["§", "Section Sign", "&sect;", 167],
    ["¶", "Pilcrow Sign", "&para;", 182],
    ["‹", "Single Left-Pointing Angle Quotation Mark", "&lsaquo;", 8249],
    ["›", "Single Right-Pointing Angle Quotation Mark", "&rsaquo;", 8250],
    ["«", "Left-Pointing Double Angle Quotation Mark", "&laquo;", 171],
    ["»", "Right-Pointing Double Angle Quotation Mark", "&raquo;", 187],
    ["‐", "Hyphen", "&#8208;", 8208],
    ["‑", "Non-Breaking Hyphen", "&#8209;", 8209],
    ["‒", "Figure Dash", "&#8210;", 8210],
    ["―", "Horizontal Bar", "&#8213;", 8213],
    ["‼", "Double Exclamation Mark", "&#8252;", 8252],
    ["⁇", "Double Question Mark", "&#8263;", 8263],
    ["⁈", "Question Exclamation Mark", "&#8264;", 8264],
    ["⁉", "Exclamation Question Mark", "&#8265;", 8265],
  ]],
  ["Legal & Reference", [
    ["©", "Copyright Sign", "&copy;", 169],
    ["®", "Registered Sign", "&reg;", 174],
    ["™", "Trade Mark Sign", "&trade;", 8482],
    ["℗", "Sound Recording Copyright", "&#8471;", 8471],
  ]],
  ["Cards, Chess & Games", [
    ["♠", "Black Spade Suit", "&spades;", 9824],
    ["♥", "Black Heart Suit", "&hearts;", 9829],
    ["♦", "Black Diamond Suit", "&diams;", 9830],
    ["♣", "Black Club Suit", "&clubs;", 9827],
    ["♤", "White Spade Suit", "&#9828;", 9828],
    ["♡", "White Heart Suit", "&#9825;", 9825],
    ["♢", "White Diamond Suit", "&#9826;", 9826],
    ["♧", "White Club Suit", "&#9831;", 9831],
    ["♔", "White Chess King", "&#9812;", 9812],
    ["♕", "White Chess Queen", "&#9813;", 9813],
    ["♖", "White Chess Rook", "&#9814;", 9814],
    ["♗", "White Chess Bishop", "&#9815;", 9815],
    ["♘", "White Chess Knight", "&#9816;", 9816],
    ["♙", "White Chess Pawn", "&#9817;", 9817],
    ["♚", "Black Chess King", "&#9818;", 9818],
    ["♛", "Black Chess Queen", "&#9819;", 9819],
    ["♜", "Black Chess Rook", "&#9820;", 9820],
    ["♝", "Black Chess Bishop", "&#9821;", 9821],
    ["♞", "Black Chess Knight", "&#9822;", 9822],
    ["♟", "Black Chess Pawn", "&#9823;", 9823],
    ["⚀", "Die Face-1", "&#9856;", 9856],
    ["⚁", "Die Face-2", "&#9857;", 9857],
    ["⚂", "Die Face-3", "&#9858;", 9858],
    ["⚃", "Die Face-4", "&#9859;", 9859],
    ["⚄", "Die Face-5", "&#9860;", 9860],
    ["⚅", "Die Face-6", "&#9861;", 9861],
    ["★", "Black Star", "&#9733;", 9733],
    ["☆", "White Star", "&#9734;", 9734],
  ]],
  ["Checks, Marks & Warnings", [
    ["✓", "Check Mark", "&#10003;", 10003],
    ["✔", "Heavy Check Mark", "&#10004;", 10004],
    ["✗", "Ballot X", "&#10007;", 10007],
    ["✘", "Heavy Ballot X", "&#10008;", 10008],
    ["☠", "Skull And Crossbones", "&#9760;", 9760],
    ["☢", "Radioactive Sign", "&#9762;", 9762],
    ["☣", "Biohazard Sign", "&#9763;", 9763],
    ["⚠", "Warning Sign", "&#9888;", 9888],
    ["⌛", "Hourglass", "&#8987;", 8987],
    ["⏳", "Hourglass With Flowing Sand", "&#9203;", 9203],
    ["⚡", "High Voltage Sign", "&#9889;", 9889],
    ["❗", "Heavy Exclamation Mark Symbol", "&#10071;", 10071],
    ["❕", "White Exclamation Mark Ornament", "&#10069;", 10069],
    ["❓", "Black Question Mark Ornament", "&#10067;", 10067],
    ["❔", "White Question Mark Ornament", "&#10068;", 10068],
    ["⁉", "Exclamation Question Mark", "&#8265;", 8265],
    ["♻", "Black Universal Recycling Symbol", "&#9851;", 9851],
    ["✴", "Eight Pointed Black Star", "&#10036;", 10036],
    ["✳", "Eight Spoked Asterisk", "&#10035;", 10035],
  ]],
  ["Weather & Nature", [
    ["☀", "Black Sun With Rays", "&#9728;", 9728],
    ["☁", "Cloud", "&#9729;", 9729],
    ["☂", "Umbrella", "&#9730;", 9730],
    ["☃", "Snowman", "&#9731;", 9731],
    ["☄", "Comet", "&#9732;", 9732],
    ["⛄", "Snowman Without Snow", "&#9924;", 9924],
    ["⛅", "Sun Behind Cloud", "&#9925;", 9925],
    ["☔", "Umbrella With Rain Drops", "&#9748;", 9748],
    ["❄", "Snowflake", "&#10052;", 10052],
    ["⛰", "Mountain", "&#9968;", 9968],
    ["🌊", "Water Wave", "&#127754;", 127754],
    ["⚡", "High Voltage Sign", "&#9889;", 9889],
  ]],
  ["Music", [
    ["♪", "Eighth Note", "&#9834;", 9834],
    ["♫", "Beamed Eighth Notes", "&#9835;", 9835],
    ["♬", "Beamed Sixteenth Notes", "&#9836;", 9836],
    ["♭", "Music Flat Sign", "&#9837;", 9837],
    ["♮", "Music Natural Sign", "&#9838;", 9838],
    ["♯", "Music Sharp Sign", "&#9839;", 9839],
    ["𝄞", "Musical Symbol G Clef", "&#119070;", 119070],
  ]],
  ["Zodiac", [
    ["♈", "Aries", "&#9800;", 9800],
    ["♉", "Taurus", "&#9801;", 9801],
    ["♊", "Gemini", "&#9802;", 9802],
    ["♋", "Cancer", "&#9803;", 9803],
    ["♌", "Leo", "&#9804;", 9804],
    ["♍", "Virgo", "&#9805;", 9805],
    ["♎", "Libra", "&#9806;", 9806],
    ["♏", "Scorpius", "&#9807;", 9807],
    ["♐", "Sagittarius", "&#9808;", 9808],
    ["♑", "Capricorn", "&#9809;", 9809],
    ["♒", "Aquarius", "&#9810;", 9810],
    ["♓", "Pisces", "&#9811;", 9811],
  ]],
  ["Gender & Misc", [
    ["♀", "Female Sign", "&female;", 9792],
    ["♂", "Male Sign", "&male;", 9794],
    ["⚥", "Male And Female Sign", "&#9893;", 9893],
    ["⚜", "Fleur-De-Lis", "&#9884;", 9884],
    ["⚕", "Staff Of Aesculapius", "&#9877;", 9877],
  ]],
  ["Fractions", [
    ["¼", "Vulgar Fraction One Quarter", "&frac14;", 188],
    ["½", "Vulgar Fraction One Half", "&frac12;", 189],
    ["¾", "Vulgar Fraction Three Quarters", "&frac34;", 190],
    ["⅓", "Vulgar Fraction One Third", "&#8531;", 8531],
    ["⅔", "Vulgar Fraction Two Thirds", "&#8532;", 8532],
    ["⅛", "Vulgar Fraction One Eighth", "&#8539;", 8539],
    ["⅜", "Vulgar Fraction Three Eighths", "&#8540;", 8540],
    ["⅝", "Vulgar Fraction Five Eighths", "&#8541;", 8541],
    ["⅞", "Vulgar Fraction Seven Eighths", "&#8542;", 8542],
    ["⅕", "Vulgar Fraction One Fifth", "&#8533;", 8533],
    ["⅖", "Vulgar Fraction Two Fifths", "&#8534;", 8534],
    ["⅗", "Vulgar Fraction Three Fifths", "&#8535;", 8535],
    ["⅘", "Vulgar Fraction Four Fifths", "&#8536;", 8536],
    ["⅙", "Vulgar Fraction One Sixth", "&#8537;", 8537],
  ]],
  ["Superscript & Subscript", [
    ["⁰", "Superscript Zero", "&#8304;", 8304],
    ["¹", "Superscript One", "&#185;", 185],
    ["²", "Superscript Two", "&#178;", 178],
    ["³", "Superscript Three", "&#179;", 179],
    ["⁴", "Superscript Four", "&#8308;", 8308],
    ["⁵", "Superscript Five", "&#8309;", 8309],
    ["⁶", "Superscript Six", "&#8310;", 8310],
    ["⁷", "Superscript Seven", "&#8311;", 8311],
    ["⁸", "Superscript Eight", "&#8312;", 8312],
    ["⁹", "Superscript Nine", "&#8313;", 8313],
    ["₀", "Subscript Zero", "&#8320;", 8320],
    ["₁", "Subscript One", "&#8321;", 8321],
    ["₂", "Subscript Two", "&#8322;", 8322],
    ["₃", "Subscript Three", "&#8323;", 8323],
    ["₄", "Subscript Four", "&#8324;", 8324],
    ["₅", "Subscript Five", "&#8325;", 8325],
    ["₆", "Subscript Six", "&#8326;", 8326],
    ["₇", "Subscript Seven", "&#8327;", 8327],
    ["₈", "Subscript Eight", "&#8328;", 8328],
    ["₉", "Subscript Nine", "&#8329;", 8329],
    ["⁺", "Superscript Plus Sign", "&#8314;", 8314],
    ["⁻", "Superscript Minus", "&#8315;", 8315],
  ]],
  ["Accented Latin Letters", [
    ["à", "Latin Small Letter A With Grave", "&agrave;", 224],
    ["á", "Latin Small Letter A With Acute", "&aacute;", 225],
    ["â", "Latin Small Letter A With Circumflex", "&acirc;", 226],
    ["ã", "Latin Small Letter A With Tilde", "&atilde;", 227],
    ["ä", "Latin Small Letter A With Diaeresis", "&auml;", 228],
    ["å", "Latin Small Letter A With Ring Above", "&aring;", 229],
    ["æ", "Latin Small Letter Ae", "&aelig;", 230],
    ["ç", "Latin Small Letter C With Cedilla", "&ccedil;", 231],
    ["è", "Latin Small Letter E With Grave", "&egrave;", 232],
    ["é", "Latin Small Letter E With Acute", "&eacute;", 233],
    ["ê", "Latin Small Letter E With Circumflex", "&ecirc;", 234],
    ["ë", "Latin Small Letter E With Diaeresis", "&euml;", 235],
    ["ì", "Latin Small Letter I With Grave", "&igrave;", 236],
    ["í", "Latin Small Letter I With Acute", "&iacute;", 237],
    ["î", "Latin Small Letter I With Circumflex", "&icirc;", 238],
    ["ï", "Latin Small Letter I With Diaeresis", "&iuml;", 239],
    ["ñ", "Latin Small Letter N With Tilde", "&ntilde;", 241],
    ["ò", "Latin Small Letter O With Grave", "&ograve;", 242],
    ["ó", "Latin Small Letter O With Acute", "&oacute;", 243],
    ["ô", "Latin Small Letter O With Circumflex", "&ocirc;", 244],
    ["õ", "Latin Small Letter O With Tilde", "&otilde;", 245],
    ["ö", "Latin Small Letter O With Diaeresis", "&ouml;", 246],
    ["ø", "Latin Small Letter O With Stroke", "&oslash;", 248],
    ["ù", "Latin Small Letter U With Grave", "&ugrave;", 249],
    ["ú", "Latin Small Letter U With Acute", "&uacute;", 250],
    ["û", "Latin Small Letter U With Circumflex", "&ucirc;", 251],
    ["ü", "Latin Small Letter U With Diaeresis", "&uuml;", 252],
    ["ý", "Latin Small Letter Y With Acute", "&yacute;", 253],
    ["ÿ", "Latin Small Letter Y With Diaeresis", "&yuml;", 255],
    ["ß", "Latin Small Letter Sharp S", "&szlig;", 223],
    ["œ", "Latin Small Ligature Oe", "&oelig;", 339],
    ["À", "Latin Capital Letter A With Grave", "&Agrave;", 192],
    ["Á", "Latin Capital Letter A With Acute", "&Aacute;", 193],
    ["Â", "Latin Capital Letter A With Circumflex", "&Acirc;", 194],
    ["Ã", "Latin Capital Letter A With Tilde", "&Atilde;", 195],
    ["Ä", "Latin Capital Letter A With Diaeresis", "&Auml;", 196],
    ["Å", "Latin Capital Letter A With Ring Above", "&Aring;", 197],
    ["Æ", "Latin Capital Letter Ae", "&AElig;", 198],
    ["Ç", "Latin Capital Letter C With Cedilla", "&Ccedil;", 199],
    ["È", "Latin Capital Letter E With Grave", "&Egrave;", 200],
    ["É", "Latin Capital Letter E With Acute", "&Eacute;", 201],
    ["Ê", "Latin Capital Letter E With Circumflex", "&Ecirc;", 202],
    ["Ë", "Latin Capital Letter E With Diaeresis", "&Euml;", 203],
    ["Ñ", "Latin Capital Letter N With Tilde", "&Ntilde;", 209],
    ["Ö", "Latin Capital Letter O With Diaeresis", "&Ouml;", 214],
    ["Ü", "Latin Capital Letter U With Diaeresis", "&Uuml;", 220],
  ]],
  ["Box Drawing", [
    ["─", "Box Drawings Light Horizontal", "&#9472;", 9472],
    ["│", "Box Drawings Light Vertical", "&#9474;", 9474],
    ["┌", "Box Drawings Light Down And Right", "&#9484;", 9484],
    ["┐", "Box Drawings Light Down And Left", "&#9488;", 9488],
    ["└", "Box Drawings Light Up And Right", "&#9492;", 9492],
    ["┘", "Box Drawings Light Up And Left", "&#9496;", 9496],
    ["├", "Box Drawings Light Vertical And Right", "&#9500;", 9500],
    ["┤", "Box Drawings Light Vertical And Left", "&#9508;", 9508],
    ["┬", "Box Drawings Light Down And Horizontal", "&#9516;", 9516],
    ["┴", "Box Drawings Light Up And Horizontal", "&#9524;", 9524],
    ["┼", "Box Drawings Light Vertical And Horizontal", "&#9532;", 9532],
    ["═", "Box Drawings Double Horizontal", "&#9552;", 9552],
    ["║", "Box Drawings Double Vertical", "&#9553;", 9553],
    ["╔", "Box Drawings Double Down And Right", "&#9556;", 9556],
    ["╗", "Box Drawings Double Down And Left", "&#9559;", 9559],
    ["╚", "Box Drawings Double Up And Right", "&#9562;", 9562],
    ["╝", "Box Drawings Double Up And Left", "&#9565;", 9565],
    ["█", "Full Block", "&#9608;", 9608],
    ["░", "Light Shade", "&#9617;", 9617],
    ["▒", "Medium Shade", "&#9618;", 9618],
    ["▓", "Dark Shade", "&#9619;", 9619],
  ]],
];
